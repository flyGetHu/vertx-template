...
public class RetryHandler {
  ...
  /** 默认重试策略 */
  private static final RetryStrategy DEFAULT_RETRY_STRATEGY = RetryStrategy.builder()
      .maxRetries(3)
      .initialDelay(1000)
      .maxDelay(30000)
      .multiplier(2.0)
  ...
  public RetryHandler(Vertx vertx,
      RetryMessagePublisher messagePublisher,
   ...
   * @return 重试结果的Future
   */
  public Future<Boolean> handleRetry(String consumerName,
    ...
    if (retryableMessage.hasExceededMaxRetries(strategy.getMaxRetries())) {
      log.warn("消费者 {} 消息重试次数已达上限 {}, 发送到死信队列",
          consumerName, strategy.getMaxRetries());
    ...
    log.info("消费者 {} 将在 {}ms 后进行第 {} 次重试",
        consumerName, delay, retryableMessage.getRetryCount());

    // 延迟重试
    return messagePublisher.publishDelayedRetryMessage(retryableMessage, queueName, delay)
        .map(true)
        .onSuccess(result -> {
          log.info("消费者 {} 重试第 {} 次已安排", consumerName, retryableMessage.getRetryCount());
        })
        .onFailure(retryError -> {
          log.error("消费者 {} 安排重试第 {} 次失败", consumerName, retryableMessage.getRetryCount(), retryError);
        });
  }
  ...
  public Future<Boolean> handleRetry(String consumerName,
    ...
    return scheduleRetry(delay, () -> retryProcessor.process(message))
        ...
        .onFailure(retryError -> {
          log.error("消费者 {} 重试第 {} 次异常", consumerName, retryableMessage.getRetryCount(), retryError);
        });
  }
   ...
   * @return 处理结果的Future
   */
  public Future<Boolean> handleDeadLetter(String consumerName, RabbitMQMessage message,
      Throwable cause) {
   ...
   */
  public void configureDeadLetter(String consumerName, DeadLetterHandler.DeadLetterConfig deadLetterConfig) {
  ...
  private long calculateDelay(RetryStrategy strategy, int retryCount) {
    long delay = (long) (strategy.getInitialDelay() * Math.pow(strategy.getMultiplier(), retryCount));
  ...
  public static class RetryStrategy {
    ...
    public String toString() {
      return String.format("RetryStrategy{maxRetries=%d, initialDelay=%dms, maxDelay=%dms, multiplier=%.1f}",
  ...
  public static class RetryStats {
    ...
    public RetryStats(String consumerName, int totalRetries, int successfulRetries, LocalDateTime lastRetryTime) {
