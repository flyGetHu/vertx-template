...
public class RetryUsageExample {
  ...
  private void configureDeadLetterQueue() {
    ...
    // 创建死信队列配置，包含自定义处理器
    DeadLetterHandler.DeadLetterConfig deadLetterConfig = new DeadLetterHandler.DeadLetterConfig(deadLetterQueueName,
  ...
  private Future<Void> handleDeadLetter(RetryableMessage retryableMessage, Throwable finalCause) {
    log.error("处理死信消息 - 消费者: {}, 失败原因: {}, 重试次数: {}",
  ...
  public void advancedRetryExample() {
    ...
    // 配置支付消费者的重试策略
    RetryHandler.RetryStrategy paymentRetryStrategy = RetryHandler.RetryStrategy.builder()
    ...
    // 配置支付死信队列
    DeadLetterHandler.DeadLetterConfig paymentDLQ = new DeadLetterHandler.DeadLetterConfig("payment.critical.dlq",
  ...
  public void queryStatsExample() {
    ...
    log.info("死信统计 - 消费者: {}, 死信总数: {}",
        dlqStats.getConsumerName(),
        dlqStats.getTotalDeadLetters());
  }
