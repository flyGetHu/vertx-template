...
public class RetryMessagePublisher {
  ...
  public Future<Void> publishRetryMessage(RetryableMessage retryableMessage, String targetQueue) {
    ...
    return getOrCreatePublisher(publisherKey)
        .compose(client -> {
          ...
          log.debug("发布重试消息到队列 {} - 消费者: {}, 重试次数: {}",
              targetQueue, retryableMessage.getConsumerName(), retryableMessage.getRetryCount());

          // 发布消息
          return client.basicPublish("", targetQueue, properties, Buffer.buffer(retryableMessage.getBody()));
        })
        .onSuccess(v -> log.debug("重试消息发布成功 - 队列: {}, 消费者: {}",
            targetQueue, retryableMessage.getConsumerName()))
        .onFailure(cause -> log.error("重试消息发布失败 - 队列: {}, 消费者: {}",
            targetQueue, retryableMessage.getConsumerName(), cause));
  }
   ...
   * @return 发布结果的Future
   */
  public Future<Void> publishDeadLetterMessage(RetryableMessage retryableMessage,
    ...
    return getOrCreatePublisher(publisherKey)
        .compose(client -> {
          // 构建死信消息属性
          AMQP.BasicProperties properties = buildDeadLetterProperties(retryableMessage, finalCause);

          log.warn("发布死信消息到队列 {} - 消费者: {}, 总失败次数: {}",
              deadLetterQueue, retryableMessage.getConsumerName(), retryableMessage.getFailureHistory().size());

          // 发布消息到死信队列
          return client.basicPublish("", deadLetterQueue, properties, Buffer.buffer(retryableMessage.getBody()));
        })
        .onSuccess(v -> log.info("死信消息发布成功 - 队列: {}, 消费者: {}",
            deadLetterQueue, retryableMessage.getConsumerName()))
        .onFailure(cause -> log.error("死信消息发布失败 - 队列: {}, 消费者: {}",
            deadLetterQueue, retryableMessage.getConsumerName(), cause));
  }
   ...
   * @return 发布结果的Future
   */
  public Future<Void> publishDelayedRetryMessage(RetryableMessage retryableMessage,
      String targetQueue,
      long delayMs) {
    // 使用定时器延迟发送
    return Future.future(promise -> {
      vertx.setTimer(delayMs, id -> {
        publishRetryMessage(retryableMessage, targetQueue)
            .onComplete(promise);
      });
    });
  }
  ...
  public Future<Void> ensureDeadLetterQueue(String deadLetterQueue) {
    ...
    return getOrCreatePublisher(publisherKey)
        .compose(client -> client.queueDeclare(deadLetterQueue, true, false, false, new JsonObject()))
  ...
  public Future<Void> closeAllPublishers() {
    ...
    for (Map.Entry<String, RabbitMQClient> entry : publisherClients.entrySet()) {
      ...
      if (client != null && client.isConnected()) {
        result = result.compose(v -> client.stop()
   ...
   * @return 消息属性BasicProperties
   */
  private AMQP.BasicProperties buildDeadLetterProperties(RetryableMessage retryableMessage,
      Throwable finalCause) {
