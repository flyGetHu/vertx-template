...
class MiddlewareChainTest {
  ...
  void setUp() {
    ...
    when(middleware1.handle(any(RoutingContext.class)))
        .thenReturn(Future.succeededFuture(MiddlewareResult.success()));
    ...
    when(middleware2.handle(any(RoutingContext.class)))
        .thenReturn(Future.succeededFuture(MiddlewareResult.success()));
  }
  ...
  void testMiddlewareExecution() {
    ...
    // 模拟中间件执行
    Future<MiddlewareResult> result = middlewareChain.execute(routingContext);

    assertNotNull(result);
    // 验证中间件被调用
    verify(middleware1, timeout(1000)).handle(routingContext);
    verify(middleware2, timeout(1000)).handle(routingContext);
  }
  ...
  void testEmptyMiddlewareChain() {
    ...
    result.onComplete(
        ar -> {
          ...
          assertEquals("无中间件需要执行", ar.result().getMessage());
        });
  }
  ...
  void testMiddlewareFailure() {
    // 测试中间件执行失败
    when(middleware1.handle(any(RoutingContext.class)))
        .thenReturn(Future.succeededFuture(MiddlewareResult.failure("401", "认证失败")));
    ...
    result.onComplete(
        ar -> {
          ...
          assertEquals("401", ar.result().getStatusCode());
          assertEquals("认证失败", ar.result().getMessage());
        });
  }
  ...
  void testMiddlewareStopChain() {
    // 测试中间件停止执行链条
    when(middleware1.handle(any(RoutingContext.class)))
        .thenReturn(Future.succeededFuture(MiddlewareResult.stop("缓存命中，停止执行")));
    ...
    result.onComplete(
        ar -> {
          assertTrue(ar.succeeded());
          assertTrue(ar.result().isSuccess());
          assertFalse(ar.result().shouldContinueChain());
          assertEquals("缓存命中，停止执行", ar.result().getMessage());
        });
  }
  ...
  void testMiddlewareException() {
    // 测试中间件抛出异常
    when(middleware1.handle(any(RoutingContext.class)))
        .thenReturn(Future.failedFuture(new RuntimeException("中间件异常")));
    ...
    result.onComplete(
        ar -> {
          assertTrue(ar.succeeded());
          assertFalse(ar.result().isSuccess());
          assertEquals("500", ar.result().getStatusCode());
          assertTrue(ar.result().getMessage().contains("中间件执行异常"));
        });
  }
}
